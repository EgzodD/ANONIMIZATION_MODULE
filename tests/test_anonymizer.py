...
class TestLocationNotHidden:
    ...
    @pytest.mark.xfail(
        reason=(
            "Предсуществующий ML-баг: PERSON-распознаватель классифицирует "
            "'Санкт-Петербург' как ФИО и скрывает его. Приватность не нарушается "
            "(данные скрыты, не утечка), но LOCATION должна была остаться в тексте. "
            "Вне зоны рефакторинга — требует дообучения/настройки модели PERSON."
        ),
        strict=False,
    )
