...
# + hard-negatives из реального прогона (модель метила их как ФИО): Королёв,
# Ясиноватая, Донецк и т.п.
TOPONYMS = [
    ...
    "Королёв", "Ясиноватая", "Донецк", "Енакиево", "Мытищи", "Балашиха",
    "Одинцово", "Подольск", "Реутов", "Химки", "Люберцы", "Домодедово",
]

# Улицы/проспекты — частый FP как ФИО («Невский проспект», «Тестовая»,
# «Центральная», «Луцкая»). Отдельная категория (hard-negatives из отчёта).
STREETS = [
    "Невский", "Тестовая", "Центральная", "Луцкая", "Артёма", "Турутина",
    "Ленина", "Советская", "Молодёжная", "Садовая", "Школьная", "Лесная",
    "Заводская", "Первомайская", "Гагарина", "Мира", "Победы", "Пушкина",
]

# Частые существительные с заглавной (как в начале предложения) — драйверы FP.
# + слова, реально помеченные моделью как ФИО в прогоне: Банковская, Спам, Алло,
# Латинская, Инвентарный.
NOUNS = [
    ...
    "Банковская", "Спам", "Алло", "Латинская", "Инвентарный", "Мобильный",
    "Электронная", "Контактный", "Основной", "Дополнительный",
]

# Организации — «ООО Ромашка», «компания Вектор»: название орг. не ФИО.
ORGS = [
    "Ромашка", "Вектор", "Горизонт", "Ориентир", "Меркурий", "Алмаз",
    "Прогресс", "Стандарт", "Технополис", "Мегастрой", "Логист", "Форвард",
]
...
NEG_TEMPLATES_NOUN = [
    ...
    "У меня {noun} карта, привязана к счёту.",
    "Нужна {noun} буква в написании адреса.",
]
# Улицы — топоним в адресном контексте, не ФИО.
NEG_TEMPLATES_STREET = [
    "Адрес: улица {street}, дом 12, квартира 5.",
    "Пункт выдачи на улице {street}.",
    "Офис переехал на {street} проспект.",
    "Доставьте на ул. {street}, дом 7.",
    "Живу на улице {street}, рядом с метро.",
]
# Организации — название компании, не ФИО.
NEG_TEMPLATES_ORG = [
    "Договор заключён с ООО {org}.",
    "Компания {org} подтвердила отгрузку.",
    "Оплата поступила от ООО {org}.",
    "Поставщик {org} задерживает доставку.",
]
...
def generate(n_topo=80, n_noun=80, n_street=70, n_org=50, n_person_topo=70):
    ex = []
    ...
    for _ in range(n_street):
        t = random.choice(NEG_TEMPLATES_STREET).replace("{street}", random.choice(STREETS))
        ex.append({"text": t, "spans": []})
    for _ in range(n_org):
        t = random.choice(NEG_TEMPLATES_ORG).replace("{org}", random.choice(ORGS))
        ex.append({"text": t, "spans": []})
