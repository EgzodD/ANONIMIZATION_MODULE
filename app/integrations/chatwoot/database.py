"""
Доступ к БД Chatwoot (только для интеграции CHATWOOT_ENABLED=true).

Engine и sessionmaker создаются ЛЕНИВО (при первом обращении), а не при
импорте модуля — иначе сервис пытался бы подключаться к БД даже в standalone
режиме. Модуль безопасно импортировать при выключенной интеграции: сам факт
импорта не создаёт соединений. Реальная попытка подключения происходит только
...
# Кэш модульного уровня — engine и sessionmaker создаются один раз, лениво.
_engine = None
_SessionLocal = None


def get_engine():
    """Возвращает engine, создавая его при первом обращении (fail-fast, если DATABASE_URL пуст)."""
    global _engine
    if _engine is None:
...
def get_session_local():
    """Возвращает sessionmaker, создавая его при первом обращении."""
